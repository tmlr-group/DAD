...
def adaptive_pgd_detector(device,
                        semantic_model,
                        clf,
                        X,
                        y,
...
def adaptive_adv_generate(semantic_model, model, sigma, sigma0, ep, test_loader, device):
    model.eval()
    ...
    with torch.enable_grad():
        for data, target in test_loader:
            ...
            x_adv = adaptive_pgd_detector(device, semantic_model, model, data, target, sigma, sigma0, ep)
...
def main():
    ...
    adaptive_detector_adv_dataset = adaptive_adv_generate(semantic_model, clf, sigma, sigma0, ep, test_loader, device)
    print('==> Save adversarial sample')
    torch.save(adaptive_detector_adv_dataset, os.path.join(PATH_DATA, f'{args.mode}_{args.attack}_{args.model}_detector.pth'))

    adaptive_detector_test_loader = DataLoader(adaptive_detector_adv_dataset, batch_size=args.mmd_batch, shuffle=False)

    print('=====================adaptive detector PGD-20 Accuracy====================')
    eval_test(denoiser, clf, device, adaptive_detector_test_loader, nat_data, semantic_model, sigma, sigma0, ep)
