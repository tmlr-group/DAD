...
class SVHN():
    ...
    def train_data(self):
        train_dataset = DATA(self.path, split='train', download=True, transform=self.transform_train())
        return DataLoader(train_dataset, batch_size=self.train_batch_size, shuffle=True)

    def test_data(self):
        test_dataset = DATA(self.path, split='test', download=True, transform=self.transform_test())
        return DataLoader(test_dataset, batch_size=self.test_batch_size, shuffle=False)

    
def main():
    train_dataset = DATA('../data/', split='train', download=True, 
    ...
    with open('../data/svhn_mmd_indices.pkl', 'wb') as f:
        pickle.dump(mmd_indices, f)

    all_indices = set(range(len(train_dataset))) 
    training_indices = list(all_indices - set(mmd_indices))
    random.shuffle(training_indices)
    with open('../data/svhn_train_indices.pkl', 'wb') as f:
